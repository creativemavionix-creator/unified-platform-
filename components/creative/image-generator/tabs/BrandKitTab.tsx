...
import { Palette, Type, Image as ImageIcon, FolderOpen, LayoutTemplate, Wand2, Plus, Save, Loader2, Sparkles, Upload, X, Eye } from 'lucide-react';
import {
  getBrandKit,
  saveBrandKit,
  suggestBrandKit,
...
function operationCost(operation: keyof typeof TOKEN_COSTS | string): number {
...
export default function BrandKitTab() {
  ...
  const handleAutoBrandGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(err instanceof Error ? err.message : 'An error occurred during brand suggestion.');
  ...
  const handleGenerateLogos = async (count: number = 4) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to generate logo variations');
    } finally {
  ...
  const handleGenerateAssets = async (assetIndex: number, count: number = 2) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to generate asset variations');
    } finally {
            ...
            <form onSubmit={handleAutoBrandGenerate} className="space-y-4">
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 flex items-center justify-center gap-1.5">
                ...
                <span className="font-bold text-purple-600 dark:text-purple-300">
                  {operationCost('brandkit/suggest') + operationCost('generate') * 5} credits
                </span>
                <span className="text-slate-400">
                  (suggest: {operationCost('brandkit/suggest')} + 5 × generate: {operationCost('generate')})
                </span>
              </p>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2 pt-4 border-t border-slate-200 dark:border-slate-800">
                ...
                >
                  ...
                  Generate 4 Logo Variations ({operationCost('generate') * 4} credits)
                </button>
              </div>
                ...
                {editedAssetPrompts.map((prompt, i) => (
                  <div key={i} className="space-y-2 p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-900/30">
                    ...
                    >
                      ...
                      Generate 2 Variations ({operationCost('generate') * 2} credits)
                    </button>
                  </div>
                ))}
              </div>
