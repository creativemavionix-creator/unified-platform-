...
import { ANALYTICS, CREDIT_USAGE, STORAGE_USAGE, PROMPT_HISTORY, MOCK_GALLERY } from '../imageGeneratorMockData';
...
export default function HistoryTab() {
  ...
  const maxStyle = Math.max(...ANALYTICS.mostUsedStyles.map((s) => s.count));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Most used styles */}
        <div className={`${card} reveal-up`}>
          <span className={label}>Most Used Styles</span>
          <div className="mt-4 space-y-3">
            {ANALYTICS.mostUsedStyles.map((s) => (
              <div key={s.name}>
                ...
                <div className="mt-1 h-2 rounded-full bg-slate-100 dark:bg-slate-900">
                  <div className="h-2 rounded-full bg-purple-600" style={{ width: `${(s.count / maxStyle) * 100}%` }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className={`${card} reveal-up lg:col-span-2`}>
          ...
          <div className="mt-3 divide-y divide-slate-100 dark:divide-slate-800">
            {MOCK_GALLERY.slice(0, 5).map((img) => (
              <div key={img.id} className="flex items-center gap-3 py-2.5">
                <img src={img.url} className="w-10 h-10 rounded-lg object-cover" alt="" />
                <div className="min-w-0 flex-1">
                  <p className="text-[12.5px] font-semibold truncate">{img.prompt}</p>
                  <p className="text-[11px] text-slate-400">{img.style} · {img.createdAt}</p>
                </div>
              </div>
            ))}
          </div>
