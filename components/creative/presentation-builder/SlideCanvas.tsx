...
type Props = {
  slide: Slide;
  /** Thumbnail strip mode — smaller type, no heavy chrome */
  compact?: boolean;
  className?: string;
  accentHex?: string;
};
...
/**
 * Renders a slide as a real 16:9 presentation surface (not a photo with floating text).
 */
export default function SlideCanvas({ slide, compact = false, className = '', accentHex = '#a78bfa' }: Props) {
  const cover = isCoverSlide(slide);
  const bullets = (slide.bullets || []).filter(Boolean).slice(0, compact ? 3 : 5);
  const accent = accentHex || '#a78bfa';
      ...
      {cover ? (
        <>
          ...
          <div className={`absolute inset-0 flex flex-col justify-end ${pad}`}>
            <p className={`${layoutCls} font-black uppercase`} style={{ color: accent }}>{slide.layout}</p>
            <h2 className={`${titleCls} font-black tracking-tight`}>{slide.title}</h2>
            {slide.subtitle ? <p className={`${subCls} font-medium`} style={{ color: `${accent}dd` }}>{slide.subtitle}</p> : null}
              ...
              <ul className={`mt-3 space-y-1.5 ${compact ? 'mt-1.5 space-y-0.5' : 'mt-4 space-y-2.5'} max-w-2xl`}>
                {bullets.map((b, i) => (
                  <li key={i} className={`flex gap-2 ${bulletCls} text-white/95`}>
                    <span className={`mt-[0.4em] shrink-0 rounded-full ${compact ? 'h-1 w-1' : 'h-1.5 w-1.5'}`} style={{ backgroundColor: accent }} />
        ...
        <>
          ...
          <div className={`absolute inset-y-0 left-0 w-[54%] flex flex-col justify-center ${pad}`}>
            <p className={`${layoutCls} font-black uppercase`} style={{ color: accent }}>{slide.layout}</p>
            <h2 className={`${titleCls} font-black tracking-tight`}>{slide.title}</h2>
            {slide.subtitle ? <p className={`${subCls} font-medium`} style={{ color: `${accent}cc` }}>{slide.subtitle}</p> : null}
