...
import { generateOutline, generateSlides, checkPresentationHealth, type OutlineSlide } from '@/lib/presentation-builder/api';
import { generateImage } from '@/lib/image-generator/api';
import { TOKEN_COSTS } from '@/lib/creative-tokens';
...
export default function CreateTab({
  ...
  const { refresh: refreshTokens } = useCreativeTokens();
  ...
  const [slideCount, setSlideCount] = useState(initialSlideCount || 10);
  const [error, setError] = useState<string | null>(null);
  const [buildProgress, setBuildProgress] = useState('');
  const [metaTemplate, setMetaTemplate] = useState(templateName || '');
  const [metaTheme, setMetaTheme] = useState(themeName || '');
  const [metaStyle, setMetaStyle] = useState(styleHint || '');
  const [ollamaStatus, setOllamaStatus] = useState<'checking' | 'ok' | 'unreachable'>('checking');
  ...
  useEffect(() => {
    ...
    checkPresentationHealth()
      .then((h) => {
        if (!alive) return;
        setOllamaStatus(h.ollama === 'ok' ? 'ok' : 'unreachable');
      })
      .catch(() => {
        if (!alive) return;
  ...
  useEffect(() => {
    // Only apply seed when it actually changes — empty string means "user should type a topic"
    if (typeof initialPrompt === 'string') setPrompt(initialPrompt);
  }, [initialPrompt]);

  useEffect(() => {
    if (initialSlideCount) setSlideCount(initialSlideCount);
  }, [initialSlideCount]);

  useEffect(() => {
    setMetaTemplate(templateName || '');
  }, [templateName]);

  useEffect(() => {
    setMetaTheme(themeName || '');
  }, [themeName]);

  useEffect(() => {
    setMetaStyle(styleHint || '');
  }, [styleHint]);

  // New template/theme seed → reset mid-flow outline so UX isn't confusing
  useEffect(() => {
    if (!(templateName || themeName || styleHint)) return;
    setStage('input');
    setOutline([]);
    setError(null);
    setDesignSuggestions([]);
  }, [templateName, themeName, styleHint]);

  const clampSlideCount = (n: number) => Math.min(40, Math.max(4, Number.isFinite(n) ? n : 10));

  const sourcePlaceholder: Record<SourceId, string> = {
    prompt: metaTemplate || metaTheme
      ? 'Enter the TOPIC of your presentation (e.g. “AI tutoring for high school math”)…'
      : 'Describe the presentation you want — audience, goal, tone...',
    topic: 'Enter a topic, e.g. "The future of renewable energy in India"',
    pdf: 'Paste the important excerpts from your PDF (text only for now).',
    doc: 'Paste content from your Word document (text only for now).',
    url: 'Paste a website URL plus any key notes you want covered.',
    existing: 'Describe the existing deck you want to remix, or paste slide titles.',
  ...
  const buildApiContent = () => {
    ...
    if (metaTemplate) parts.push(`Format/template: ${metaTemplate} (structure only — do not paste into slide text)`);
    if (metaTheme) parts.push(`Visual theme: ${metaTheme} (style only — do not paste into slide text)`);
    if (metaStyle) parts.push(`Visual/style guidance: ${metaStyle} (style only — do not paste into slide text)`);
    return parts.join('\n');
  };
  ...
  const handleGenerateOutline = async () => {
    const topic = prompt.trim();
    if (!topic) {
      setError(metaTemplate || metaTheme
        ? 'Enter a topic for this template/theme before generating.'
        : 'Add your input before generating an outline.');
      return;
    }
    if (FILE_SOURCES.includes(source) && topic.length < 20) {
      setError('For file sources, paste extracted text for now (min ~20 characters).');
      return;
    }

    const content = buildApiContent();
    setError(null);
    setStage('generating');
    try {
      ...
      setGenSource(result.generation_source || null);
      setStage('outline');
      void refreshTokens();
    } catch (err) {
  ...
  const handleGenerateSlides = async () => {
    ...
    try {
      ...
      const usingOllama = result.generation_source === 'ollama' || result.generation_source === 'mixed';
      setBuildProgress(
        usingOllama
          ? `Generating ${result.slides.length} visuals from image API…`
          : `Structured draft ready — generating ${result.slides.length} visuals…`,
      );
      const imageResults = await Promise.all(
        result.slides.map(async (s, i) => {
          try {
            ...
            return PLACEHOLDER_THUMBS[i % PLACEHOLDER_THUMBS.length];
          }
        }),
      );

      const built: Slide[] = result.slides.map((s, i) => ({
        id: s.id,
        title: s.title,
        layout: s.layout,
        thumb: imageResults[i],
        notes: s.notes || '',
        subtitle: s.subtitle || '',
        body: s.body || '',
        bullets: s.bullets,
  ...
  return (
    <div className="space-y-8">
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-[#0c0c14]/70 px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <p className="text-[12px] font-semibold text-slate-600 dark:text-slate-300">
          Flow: <span className="text-slate-900 dark:text-white">Topic</span>
          {' → '}
          <span className="text-slate-900 dark:text-white">Outline</span>
          {' → '}
          <span className="text-slate-900 dark:text-white">Slides + images</span>
          {' → '}
          <span className="text-slate-900 dark:text-white">Edit / Download</span>
        </p>
        <p
        ...
        >
          {ollamaStatus === 'ok'
            ...
            : ollamaStatus === 'checking'
              ? 'Checking Ollama…'
              : 'Ollama offline — start Ollama + llama3:8b for live AI copy'}
        </p>
      </div>
      ...
      {genSource && genSource !== 'ollama' && (stage === 'outline' || stage === 'building') && (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs font-semibold text-amber-800 dark:border-amber-900/50 dark:bg-amber-950/20 dark:text-amber-300">
          {genSource === 'fallback'
            ? 'Structured draft in use (Ollama was unavailable or returned incomplete JSON). Start Ollama with llama3:8b and regenerate for live AI wording.'
            : 'Partially generated with Ollama — some slides used structured fallback copy.'}
        </div>
      )}

      {(stage === 'input' || stage === 'generating') && (
        <>
          {(metaTemplate || metaTheme || metaStyle) && (
            <div className="rounded-2xl border border-purple-200 dark:border-purple-900/50 bg-purple-50/70 dark:bg-purple-500/10 px-4 py-3 text-[12px] font-semibold text-purple-700 dark:text-purple-300 space-y-1">
              {metaTemplate && <p>Template: {metaTemplate}</p>}
              {metaTheme && <p>Theme: {metaTheme}</p>}
              {metaStyle && !metaTheme && <p>Style: {metaStyle}</p>}
              <p className="font-medium text-purple-600/80 dark:text-purple-200/80">
                Enter your topic below — template/theme only controls structure and look, not the slide wording.
              </p>
            </div>
          )}
          <div>
          ...
          <div className={card}>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onBlur={() => setSlideCount((n) => clampSlideCount(n))}
            ...
            <p className="mt-4 text-[11px] text-slate-500">
              ...
              <span className="ml-2 text-slate-400">· Ctrl/⌘+Enter to generate</span>
            </p>
