...
import {
  QUICK_TEMPLATES, FEATURED_THEMES, AI_SUGGESTIONS, ANALYTICS,
} from '../presentationMockData';
import type { CreateSeed } from '../PresentationWorkspace';
import {
  toggleFavorite,
  type ActivityItem,
...
type AnalyticsShape = ReturnType<typeof import('@/lib/presentation-builder/store').computeAnalytics>;

type Props = {
  onNavigate: (tab: string) => void;
  onStartCreate?: (seed?: CreateSeed) => void;
...
export default function DashboardTab({
  ...
  const favorites = decks.filter((p) => p.favorite);
  const totals = analytics?.totals || ANALYTICS.totals;
  const notifs = notifications.length
    ? notifications
    : [
        { id: 'n0', text: 'Generate a deck to unlock live notifications.', time: '—', unread: false, createdAtMs: 0 },
      ];
  const acts = activity.length
    ? activity
    : [{ id: 'a0', text: 'Your activity will show up here after you generate or export.', time: '—', createdAtMs: 0 }];

  const stats = [
    { label: 'Presentations', value: totals.totalPresentations, icon: FolderKanban },
    { label: 'Slides Generated', value: totals.totalSlidesGenerated.toLocaleString(), icon: Layers },
    { label: 'Exports', value: totals.totalExports, icon: TrendingUp },
    { label: 'Collaborators', value: totals.activeCollaborators, icon: Star },
  ];

  const startFromHero = () => {
    onStartCreate?.({ prompt: heroPrompt.trim() || undefined });
  };
    ...
    <div className="space-y-8">
      <div className={`${card} relative overflow-hidden`}>
        ...
        <p className="mt-2 text-[12.5px] text-slate-500 dark:text-slate-400">
          1) Enter a topic → 2) Review outline → 3) Build slides &amp; images → 4) Edit, brand, export
        </p>
        <div className="mt-5 flex flex-col sm:flex-row gap-3">
          <input
            value={heroPrompt}
            onChange={(e) => setHeroPrompt(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && startFromHero()}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className={`${card} lg:col-span-2`}>
          <div className="flex items-center justify-between mb-4">
            ...
            <button type="button" onClick={() => onNavigate('templates')} className="text-[11px] font-bold text-purple-600 dark:text-purple-300 flex items-center gap-1">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {QUICK_TEMPLATES.map((t) => (
              <button
                key={t.id}
                type="button"
                onClick={() => onStartCreate?.({ prompt: t.prompt, slideCount: 10, templateName: t.label, styleHint: `${t.label} structure` })}
          ...
          <div className="mt-4 space-y-3">
            {AI_SUGGESTIONS.map((s, i) => (
              <button
                key={i}
                type="button"
                onClick={() => onStartCreate?.({ prompt: s })}
                className="flex gap-2 text-left text-[12.5px] text-slate-600 dark:text-slate-300 hover:text-purple-600 dark:hover:text-purple-300"
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className={`${card} lg:col-span-2`}>
          <div className="flex items-center justify-between mb-4">
            ...
            <button type="button" onClick={() => onNavigate('history')} className="text-[11px] font-bold text-purple-600 dark:text-purple-300 flex items-center gap-1">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {!decks.length && (
              <button
                ...
                className="col-span-2 sm:col-span-3 rounded-xl border border-dashed border-slate-300 dark:border-slate-700 px-4 py-8 text-center text-[12.5px] font-semibold text-slate-500 hover:border-purple-400"
              >
                No presentations yet — generate your first deck →
              </button>
            )}
            {decks.slice(0, 6).map((p) => (
              <button
                key={p.id}
                type="button"
                onClick={() => onOpenDeck?.(p)}
                className="text-left rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 hover:border-purple-300 dark:hover:border-purple-700 hover:-translate-y-0.5 transition-all"
              >
                ...
                <div className="p-2.5">
                  ...
                  <p className="text-[10.5px] text-slate-400">
                    {p.slideCount} slides · {p.updatedAt}
                  </p>
                </div>
              </button>
            ))}
          ...
          <div className="mt-4 grid grid-cols-3 gap-2.5">
            {FEATURED_THEMES.filter((t) => t.popular).map((t) => (
              <button
                key={t.id}
                type="button"
                onClick={() => onStartCreate?.({ prompt: '', themeId: t.id, themeName: t.name, styleHint: `${t.name} visual style` })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className={card}>
          ...
          <div className="mt-4 space-y-2.5">
            {!favorites.length && !decks.length && (
              <p className="text-[12.5px] text-slate-400">Star a deck after you generate one.</p>
            )}
            {(favorites.length ? favorites : decks.slice(0, 3)).map((p) => (
              <div key={p.id} className="flex items-center gap-3 rounded-xl border border-slate-100 dark:border-slate-800 p-2.5 hover:border-purple-200 dark:hover:border-purple-800 transition-colors">
                <button type="button" className="flex items-center gap-3 min-w-0 flex-1 text-left" onClick={() => onOpenDeck?.(p)}>
                ...
                <button type="button" onClick={() => toggleFavorite(p.id)} aria-label="Toggle favorite">
                  <Star size={14} className={p.favorite ? 'fill-yellow-400 text-yellow-400' : 'text-slate-400'} />
                </button>
              </div>
            ))}
          </div>
