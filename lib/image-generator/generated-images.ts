...
export function useGeneratedImages() {
    ...
    try {
      ...
      setImages(
        (await listImages()).map((image) => ({
          ...
          favorite: selected.has(image.filename),
        }))
      );
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Could not load generated images.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const toggleFavorite = (filename: string) => {
    ...
    localStorage.setItem(favoritesKey, JSON.stringify(Array.from(selected)));
    setImages((current) =>
      current.map((image) =>
        image.filename === filename ? { ...image, favorite: !image.favorite } : image
      )
    );
  };
