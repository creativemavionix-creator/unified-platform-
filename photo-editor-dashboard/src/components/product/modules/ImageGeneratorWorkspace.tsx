...
import {
  ...
  BarChart3, Sparkles, ChevronDown, Sun, Moon,
} from 'lucide-react';
...
import { getTokenBalance, DAILY_TOKEN_LIMIT, formatTimeRemaining } from '../../../lib/tokens';
...
export default function ImageGeneratorWorkspace({ onViewChange, theme, onThemeToggle }: Props) {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key?.startsWith('mvx_creative_tokens_')) {
        refreshTokenBalance();
      }
    };
    window.addEventListener('storage', handleStorageChange);
    
    // Also listen for same-tab changes (dispatch custom event)
    const handleLocalChange = () => refreshTokenBalance();
    window.addEventListener('mvx-tokens-changed', handleLocalChange);
    
    // Refresh when window gains focus (user switches back to tab)
    const handleVisibilityChange = () => {
      if (!document.hidden) refreshTokenBalance();
    };
    ...
    return () => {
      ...
      window.removeEventListener('mvx-tokens-changed', handleLocalChange);
  ...
  return (
    <div className="w-full min-h-screen bg-slate-50 dark:bg-[#07070f] text-slate-900 dark:text-white flex">
      ...
      <aside className="hidden lg:sticky lg:top-o lg:z-0 lg:flex h-screen w-72 shrink-0 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0b0b14] flex-col">
        ...
        <div className="mx-4 mb-4 rounded-2xl border border-purple-100 dark:border-purple-900/40 bg-purple-50/60 dark:bg-purple-500/5 p-4">
          ...
          <p className="mt-1 text-lg font-black">{tokenBalance.used} <span className="text-xs font-semibold text-slate-400">/ {DAILY_TOKEN_LIMIT}</span></p>
          <div className="mt-2 h-1.5 w-full rounded-full bg-purple-100 dark:bg-purple-500/15">
            <div className="h-1.5 rounded-full bg-purple-600" style={{ width: `${Math.min(100, (tokenBalance.used / DAILY_TOKEN_LIMIT) * 100)}%` }} />
          </div>
     ...
     <div className="flex-1 min-w-0 lg:ml-auto">
        ...
        <header className="sticky top-0 z-30 flex items-center justify-between gap-2 sm:gap-4 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-[#07070f]/80 backdrop-blur px-3 sm:px-6 py-3.5">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 relative" ref={navMenuRef}>
            ...
            {/* Only show tab name on mobile where sidebar is hidden */}
            <h1 className="lg:hidden text-sm sm:text-base font-black uppercase tracking-wide truncate">
