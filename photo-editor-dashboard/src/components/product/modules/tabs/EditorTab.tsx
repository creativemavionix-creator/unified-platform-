...
import {
  ...
  RotateCw, Sun, Contrast, Droplet, ImagePlus, Mic, AlertCircle,
} from 'lucide-react';
import {
  applyStyle,
  enhanceFace,
...
import { type GeneratedImage, useGeneratedImages } from '../../../../lib/generated-images';
import { parseVoiceCommand, type VoiceTool } from '../../../../lib/voice-commands';
...
export default function EditorTab() {
  ...
  const runTool = async (toolToRun = activeTool, styleToUse = style) => {
    ...
    const operation = TOOL_TO_OPERATION[toolToRun];
    if (operation) {
      const cost = getOperationCost(operation);
      const tokenCheck = checkDemoTokens(cost);
      if (!tokenCheck.ok) {
        setError(tokenCheck.error || 'Not enough credits for this operation');
        return;
      }
    }
    ...
    try {
      ...
      if (operation) {
        deductDemoTokens(getOperationCost(operation));
      }
    } catch (error) {
    ...
    <div className="grid grid-cols-1 xl:grid-cols-[280px_1fr_300px] gap-6">
      ...
      <div className={`${card} reveal-up`}>
        ...
        <div className="mt-3 space-y-1.5">
          {AI_TOOLS.map((t) => {
            ...
            const operation = TOOL_TO_OPERATION[t.id];
            const cost = operation ? getOperationCost(operation) : 0;
            return (
              <button
                key={t.id}
                onClick={() => t.available && setActiveTool(t.id)} disabled={!t.available}
                className={`w-full flex items-center justify-between gap-2.5 rounded-xl px-3 py-2.5 text-[12.5px] font-semibold transition-colors ${
              ...
              >
                ...
                {cost > 0 && <span className="text-[10px] font-bold text-purple-600 dark:text-purple-300 bg-purple-50 dark:bg-purple-500/10 px-2 py-0.5 rounded-full">{cost} credits</span>}
              </button>
            );
          })}
        </div>
      ...
      <div className={`${card} reveal-up`}>
        <div className="flex items-center justify-between">
          ...
          {(() => {
            const operation = TOOL_TO_OPERATION[activeTool];
            const cost = operation ? getOperationCost(operation) : 0;
            return cost > 0 ? (
              <span className="mr-2 inline-flex items-center gap-1.5 text-[10px] font-bold text-purple-600 dark:text-purple-300 bg-purple-50 dark:bg-purple-500/10 px-2.5 py-1 rounded-full">
